...
def get_variables(
    ...
    """
    Return only the explicit xhat/yhat URC policy dimensions.

    gaussian_state is an internal Knowledge variable and does NOT
    become an EvoChecker decision dimension. The controller therefore remains
    position dependent with exactly decision_x_y variables.
    """
    ...
    if estimates != ["xhat", "yhat"]:
        raise ValueError(
            "Position-local exact Gaussian model must expose xhat and yhat "
            "as explicit Knowledge variables."
        )
...
def add_controller(
    ...
    with open(
        ...
        for combination in combinations:
            ...
            # Position-local exact Gaussian representation exposes xhat/yhat directly.
            # gaussian_state deliberately does not appear here:
            # the URC policy remains one decision per estimated position.
            parts = [
                f"{estimate}={value}"
