...
Belief abstraction
------------------
The raw HMM belief beta is a 361-dimensional probability vector. Reachable
beliefs are generated offline. Every distinct reachable belief receives its
own finite integer hstate, i.e. hstate <-> beta is lossless.

K clustering is NOT part of the final HMM model. It can optionally be run only
as a compression experiment via --run-k-sweep.
...
# ---------------------------------------------------------------------------
# Exact reachable HMM-belief states (NO clustering)
# ---------------------------------------------------------------------------

def build_exact_hmm_states(
    model: dict,
    beliefs: np.ndarray,
    records: Sequence[RawRecord],
    unique_mse: np.ndarray,
    ...
    """
    Build the finite HMM knowledge-state model used by PRISM.

    IMPORTANT:
    ----------
    Each hstate is exactly one distinct reachable HMM belief beta.
    There is NO clustering and therefore NO lossy beta -> representative
    approximation.

        hstate <-> beta

    The 361 hidden states are the support of beta, while hstate is the observer
    knowledge state used by PARLEY/PRISM.
    """
    pi = build_initial_belief(model)

    # Find exact reset belief pi and force it to hstate 0.
    pi_distance = np.max(np.abs(beliefs - pi[None, :]), axis=1)
    pi_uid = int(np.argmin(pi_distance))
    if float(pi_distance[pi_uid]) > 1e-12:
        raise ValueError(
            "Initial belief pi was not found among reachable beliefs."
        )

    ordered_uids = [pi_uid] + [
        uid for uid in range(len(beliefs)) if uid != pi_uid
    ]

    hstate_of_uid = np.empty(len(beliefs), dtype=int)
    for hstate, uid in enumerate(ordered_uids):
        hstate_of_uid[uid] = hstate

    occurrence_counts = Counter(r.belief_uid for r in records)

    states = []
    for hstate, uid in enumerate(ordered_uids):
        beta = beliefs[uid]
        mse = float(unique_mse[uid])
        level = int(uncertainty_level(mse, thresholds))

        states.append({
            "hstate": int(hstate),
            "belief_state": int(hstate),  # compatibility with generator
            "raw_belief_uid": int(uid),
            "occurrences": int(occurrence_counts.get(uid, 0)),
            "mse": mse,
            "urc_level": level,
    ...
    # Exact context-dependent hstate transitions.
    # For prediction-only filtering:
    #   beta_next = beta A[xhat,yhat,action]
    # Hence the next hstate is deterministic.
    transitions = {}

    for r in records:
        if r.action is None or r.successor_belief_uid is None:
            continue

        current_hstate = int(hstate_of_uid[r.belief_uid])
        next_hstate = int(hstate_of_uid[r.successor_belief_uid])

        key = f"{r.xhat},{r.yhat},{current_hstate}"
        value = {
            "action": r.action,
            "next_belief_state": next_hstate,
        ...
        if previous is not None and previous != value:
            raise ValueError(
                "The same exact (xhat,yhat,hstate) context produced "
                f"different successors: {key}: {previous} vs {value}"
            )

        transitions[key] = value

    # Determine contexts that can occur as successors but have no outgoing
    # transition in the offline table (normally max-step horizon / target).
    n = int(model["n"])
    available_contexts = set()
    successor_contexts = set()

    for key, value in transitions.items():
        x_s, y_s, h_s = key.split(",")
        xhat = int(x_s)
        yhat = int(y_s)
        hstate = int(h_s)
        action = value["action"]
        next_hstate = int(value["next_hstate"])

        available_contexts.add((xhat, yhat, hstate))
        next_xhat, next_yhat = move_estimate(xhat, yhat, action, n)
        successor_contexts.add((next_xhat, next_yhat, next_hstate))

    frontier_contexts = sorted(successor_contexts - available_contexts)

    payload = {
        "map_id": int(model["map_id"]),
        "mode": "exact_reachable_beliefs",
        "hidden_state_count": int(len(model["hidden_states"])),
        "belief_state_count": int(len(states)),
        "hmm_state_count": int(len(states)),
        ...
        "states": states,
        "representatives": states,  # compatibility alias; these are NOT clustered
        ...
        "frontier_contexts": [
            ...
            for x, y, h in frontier_contexts
        ],
        "filtering_semantics": {
            "between_updates": (
                "beta_next = beta * A[xhat,yhat,action]"
            ...
            "on_update": "beta = pi",
            "hstate_semantics": (
                "lossless ID of one exact reachable HMM belief"
            ),
        },
        "metrics": {
            "belief_approximation_error": 0.0,
            "mse_abs_error": 0.0,
            "mse_rel_error": 0.0,
            "urc_level_disagreement": 0.0,
            "transition_mismatch_rate": 0.0,
        },
    }
...
def analyse_age_position_exact(
    records: Sequence[RawRecord],
    unique_mse: np.ndarray,
    thresholds: Sequence[float],
):
    """
    Expressivity analysis on the exact raw HMM beliefs, before any optional
    compression.

    High variation here can be GOOD: it means the HMM is not merely an age
    counter and may contain history-dependent knowledge.
    """
    age_groups = defaultdict(list)
    conditioned_groups = defaultdict(list)

    for r in records:
        mse = float(unique_mse[r.belief_uid])
        level = int(uncertainty_level(mse, thresholds))
        item = (int(r.belief_uid), mse, level)

        age_groups[int(r.age)].append(item)
        conditioned_groups[
            (int(r.age), int(r.xhat), int(r.yhat))
        ].append(item)

    age_rows = []
    for age in sorted(age_groups):
        items = age_groups[age]
        mses = np.asarray([x[1] for x in items], dtype=float)
        levels = [x[2] for x in items]
        level_counts = Counter(levels)
        modal_count = max(level_counts.values())

        age_rows.append({
            "age": int(age),
            "occurrences": int(len(items)),
            "unique_beliefs": int(len({x[0] for x in items})),
            "unique_urc_levels": int(len(level_counts)),
            "mse_mean": float(np.mean(mses)),
            "mse_std": float(np.std(mses)),
            "mse_min": float(np.min(mses)),
            "mse_max": float(np.max(mses)),
            "mse_range": float(np.max(mses) - np.min(mses)),
            "modal_level_disagreement": float(
                1.0 - modal_count / len(items)
            ),
        })

    position_rows = []
    for (age, xhat, yhat), items in sorted(conditioned_groups.items()):
        if len(items) < 2:
            continue

        mses = np.asarray([x[1] for x in items], dtype=float)
        belief_ids = {x[0] for x in items}
        levels = [x[2] for x in items]
        level_counts = Counter(levels)
        modal_count = max(level_counts.values())
        ...
        position_rows.append({
            ...
            "yhat": int(yhat),
            "occurrences": int(len(items)),
            "unique_beliefs": int(len(belief_ids)),
            "unique_urc_levels": int(len(level_counts)),
            "mse_mean": float(np.mean(mses)),
            "mse_std": float(np.std(mses)),
            "mse_min": float(np.min(mses)),
    ...
    n_groups = len(position_rows)
    multi_belief = sum(
        row["unique_beliefs"] > 1 for row in position_rows
    )
    multi_level = sum(
        row["unique_urc_levels"] > 1 for row in position_rows
    )

    summary = {
        "age_group_count": int(len(age_rows)),
        "same_age_position_group_count": int(n_groups),
        "same_age_position_multi_belief_groups": int(multi_belief),
        "same_age_position_multi_belief_fraction": float(
            multi_belief / n_groups if n_groups else 0.0
        ),
        "same_age_position_multi_level_groups": int(multi_level),
        "same_age_position_multi_level_fraction": float(
            multi_level / n_groups if n_groups else 0.0
        ),
    }

    return age_rows, position_rows, summary


def write_exact_outputs(
    map_dir: Path,
    exact_payload: dict,
    age_rows: Sequence[dict],
    position_rows: Sequence[dict],
    analysis_summary: dict,
):
    exact_path = map_dir / "exact_reachable_beliefs.json"
    with exact_path.open("w", encoding="utf-8") as f:
        json.dump(exact_payload, f, indent=2)

    age_path = map_dir / "age_analysis.csv"
    with age_path.open("w", newline="", encoding="utf-8") as f:
        if age_rows:
            writer = csv.DictWriter(
                f, fieldnames=list(age_rows[0].keys())
            )
            writer.writeheader()
            writer.writerows(age_rows)

    pos_path = map_dir / "age_position_analysis.csv"
    with pos_path.open("w", newline="", encoding="utf-8") as f:
        if position_rows:
            writer = csv.DictWriter(
                f, fieldnames=list(position_rows[0].keys())
            )
            writer.writeheader()
            writer.writerows(position_rows)

    analysis_path = map_dir / "age_position_analysis.json"
    with analysis_path.open("w", encoding="utf-8") as f:
        json.dump({
            "summary": analysis_summary,
            "age_groups": list(age_rows),
            "same_age_position_groups": list(position_rows),
        }, f, indent=2)

    return exact_path, age_path, pos_path, analysis_path
...
def process_map(
    ...
    print(f"\nMap {model['map_id']}: generating exact reachable HMM beliefs ...")
    ...
    print(
        ...
        f"exact unique HMM beliefs={len(beliefs)}"
    )

    squared_errors = hidden_squared_errors(model)
    unique_mse = belief_mse(beliefs, squared_errors)

    # The URC remains c in [1..10], so only the first ten age reference
    # thresholds are needed even if exact beliefs are generated to 20 steps.
    threshold_steps = min(10, max_steps)
    thresholds = monotone_age_thresholds(
        records=records,
        unique_mse=unique_mse,
        max_steps=threshold_steps,
    )

    map_dir = output_root / f"map_{model['map_id']}"
    map_dir.mkdir(parents=True, exist_ok=True)

    exact_payload = build_exact_hmm_states(
        model=model,
        beliefs=beliefs,
        records=records,
        unique_mse=unique_mse,
    ...
    age_rows, position_rows, analysis_summary = (
        analyse_age_position_exact(
            records=records,
            unique_mse=unique_mse,
            thresholds=thresholds,
        )
    ...
    exact_path, age_path, pos_path, analysis_path = (
        write_exact_outputs(
            map_dir=map_dir,
    ...
    print(
        f"  HMM states={exact_payload['hmm_state_count']} "
        "(one state per exact reachable belief)"
    )
    print(
        "  same-(age,xhat,yhat): "
        f"groups={analysis_summary['same_age_position_group_count']}, "
        f"multi-belief="
        f"{100*analysis_summary['same_age_position_multi_belief_fraction']:.2f}%, "
        f"multi-level="
        f"{100*analysis_summary['same_age_position_multi_level_fraction']:.2f}%"
    )
    ...
    # K-clustering is now optional and is NOT part of the final HMM model.
    summaries = []
    if run_k_sweep:
        error_points = error_points_from_model(model)

        print(
            f"  optional K sweep: building SW2 embedding "
            f"({n_projections}x{n_quantiles}) ..."
        )
    ...
    return {
        "map_id": int(model["map_id"]),
        "hmm_states": int(exact_payload["hmm_state_count"]),
        "raw_occurrences": int(len(records)),
        **analysis_summary,
    }
...
def precompute_maps(
    ...
    for map_id in range(first_map, last_map + 1):
        ...
        all_exact_summaries.append(summary)

    if all_exact_summaries:
        output_dir.mkdir(parents=True, exist_ok=True)

        combined_path = output_dir / "all_maps_exact_hmm_summary.csv"
        fieldnames = list(all_exact_summaries[0].keys())
        ...
        with combined_path.open(
            ...
            writer.writeheader()
            writer.writerows(all_exact_summaries)

        print(f"\nCombined exact-HMM summary: {combined_path}")


def main():
    parser = argparse.ArgumentParser(
        description=(
            "Generate exact reachable HMM belief states for PRISM. "
            "K clustering is optional and only for compression analysis."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also run the old K-clustering compression experiment. "
            "The exact HMM states are always generated regardless."
        ),
    )
