...
GAUSSIAN_METRIC = "w2_mse1_level1"
GAUSSIAN_MODEL_DIR = Path("gaussian_refined_models")
...
def preambel():
    with open(
        ...
        # The actual Gaussian MSE thresholds remain offline information.
        ...
        # threshold level reached by its raw MSE.
        ...
        for representative in gaussian_model[
            "representatives"
        ]:
            state_id = int(
            ...
            if not (
                ...
                raise ValueError(
                    f"Invalid Gaussian uncertainty level "
                    f"{reached_level} for state {state_id}."
                )
        ...
        # Semantics:
        # selected URC level c means the raw MSE threshold threshold[c].
        # A Gaussian state whose uncertainty has reached level L requires
        # an update iff c <= L.
        # This is exactly equivalent to:
        #   MSE(gstate) >= raw_threshold[c]
...
def knowledge():
    with open(
        ...
        # exact position and exact Gaussian error mu=0,Sigma=0 = gstate 0.
        f.write(
...
def load_precomputed_gaussian_model(map_id):
    ...
    Expected file:
        gaussian_refined_models/map_<id>.json
    """
    path = (
        GAUSSIAN_MODEL_DIR
        / f"map_{map_id}.json"
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run refine_gaussian_mse_representatives.py first."
        )
    ...
    if model.get("uncertainty_metric") != "mse":
        raise ValueError(
            f"{path}: expected uncertainty_metric='mse', "
            f"got {model.get('uncertainty_metric')!r}."
        )

    if model.get("model_type") != "gaussian_mse_refined":
        raise ValueError(
            f"{path}: expected model_type='gaussian_mse_refined', "
            f"got {model.get('model_type')!r}."
        )

    if model.get("metric") != GAUSSIAN_METRIC:
        raise ValueError(
            f"{path}: expected metric={GAUSSIAN_METRIC!r}, "
            f"got {model.get('metric')!r}."
        )

    if int(model["state_count"]) != GAUSSIAN_K:
        raise ValueError(
            f"{path}: expected exactly {GAUSSIAN_K} refined states, "
            f"got {model['state_count']}."
        )
    ...
    # Representative IDs, MSE values, levels and reset state.
    thresholds = [
        float(value)
        for value in model["thresholds"]
    ]

    for expected_id, representative in enumerate(
        ...
        if state_id != expected_id:
            raise ValueError(
                f"{path}: representatives must be ordered by state_id; "
                f"expected {expected_id}, got {state_id}."
            )

        mse = (
            float(representative["var_x"])
            + float(representative["var_y"])
            + float(representative["mu_x"]) ** 2
            + float(representative["mu_y"]) ** 2
        )
        ...
        if abs(
            ...
            raise ValueError(
                f"{path}: inconsistent MSE for state {state_id}."
            )
        ...
        if int(
            ...
            raise ValueError(
                f"{path}: inconsistent uncertainty level for "
                f"state {state_id}."
            )
    ...
    for field in (
        ...
        if abs(float(zero[field])) > 1e-15:
            raise ValueError(
                f"{path}: representative 0 is not exact mu=0, Sigma=0."
            )

    for key, transition in model[
        "transitions"
    ].items():
        try:
            x, y, state_id = [
            ...
            raise ValueError(
                f"{path}: invalid transition key {key!r}."
            ) from exc
        ...
        if not (
            ...
            raise ValueError(
                f"{path}: transition key out of range: {key}."
            )
        ...
        if action not in directions:
            raise ValueError(
                f"{path}: invalid action {action!r} in {key}."
            )
        ...
        if not (
            ...
            raise ValueError(
                f"{path}: invalid next_state={next_state} in {key}."
            )
