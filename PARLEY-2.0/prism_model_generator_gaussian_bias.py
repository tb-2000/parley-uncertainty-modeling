...
def preambel():
    with open(
        ...
        # absolute magnitude of Gaussian MSE.
        for level in range(0, 11):
...
def load_precomputed_gaussian_model(map_id):
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run full_gaussian_representatives_bias.py first."
        )
    ...
    if model["metric"] != "wasserstein":
        raise ValueError(
            f"{path}: expected metric='wasserstein', got {model['metric']!r}."
        )

    if model["uncertainty_metric"] != "mse":
        raise ValueError(
            f"{path}: expected uncertainty_metric='mse', "
            f"got {model['uncertainty_metric']!r}."
        )
    ...
    for field in ("bias_x", "bias_y", "var_x", "var_y", "cov_xy"):
        if abs(float(zero[field])) > 1e-15:
            raise ValueError(
                f"{path}: representative 0 is not exact bias=0, Sigma=0."
            )
