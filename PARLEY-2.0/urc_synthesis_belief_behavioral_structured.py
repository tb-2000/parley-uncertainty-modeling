...
def get_variables(prism_model_path, decision_variables):
    ...
    The structured behavioral model exposes xhat/yhat explicitly again.
    belief_state and substate are Knowledge-internal and must NOT become
    EvoChecker decision dimensions.
    """
    ...
    for match in int_variable_declaration_pattern.finditer(text):
        ...
        # Only estimated position determines a URC decision variable.
        ...
        variables.append([name, lower_limit, upper_limit])
        estimates.append(name)

    if estimates != ["xhat", "yhat"]:
        raise ValueError(
            "Structured behavioral model must declare xhat and yhat "
...
def add_controller(
    ...
    with open(file_path, 'a') as file:
        ...
        for combination in combinations:
            ...
            # Structured model: use the explicit estimated position.
            parts = [
                f'{estimate}={value}'
