...
def manipulate_prism_model(
    ...
    HMM semantics
    -------------
    If the input model contains an position-local exact HMM-belief variable

        hmm_state : [0..HMM_STATES-1] ...

    hmm_state is deliberately NOT made a decision dimension.  The URC still
    ...
    and avoids a combinatorial decision variable decision_x_y_hmm_state.
    """
    ...
    # hmm_state is the knowledge state used to evaluate uncertainty, not a
    ...
    if hmm_mode and any(
        name in decision_variables
        for name in ('hmm_state',)
    ):
        print(
            "HMM: ignoring 'hmm_state' as a URC decision dimension; "
            "the URC remains position-dependent and hmm_state determines "
...
def has_hmm_state(prism_model_path):
    pattern = re.compile(r'\bhmm_state\s*:\s*\[')
...
def get_variables(prism_model_path, decision_variables):
    ...
    hmm_state is never inferred as a decision dimension.
    """
