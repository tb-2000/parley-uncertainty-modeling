...
def remove_counter_from_module(
    ...
    """
    Remove the generator's fixed initial threshold constant.
    In the UMC it becomes a mutable URC variable.
    """
    pattern = re.compile(
        r"^\s*const\s+int\s+"
        r"max_gaussian_uncertainty\s*=\s*\d+\s*;"
    )
...
def add_controller(
    ...
    with open(
        ...
        file.write(
            ...
            "max Gaussian trace uncertainty\n"
        )
